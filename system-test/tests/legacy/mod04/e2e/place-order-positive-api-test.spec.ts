...
test('shouldPlaceOrderForValidInput', async ({ eventsCompanionApiClient, erpClient }) => {
    ...
    const result = await eventsCompanionApiClient.orders().placeOrder({ sku, quantity: '5', country: 'US' });
    ...
    if (result.success) {
        ...
        const viewResult = await eventsCompanionApiClient.orders().viewOrder(result.value.orderNumber);
