...
test('shouldRejectOrderWithNonIntegerQuantity', async ({ eventsCompanionApiClient }) => {
    // When: place order with non-integer quantity via API client
    const result = await eventsCompanionApiClient.orders().placeOrder({ sku: 'SOME-SKU', quantity: 'invalid-quantity', country: 'US' });
