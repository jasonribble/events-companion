...
export class EventsCompanionUiClient {
  ...
  async openHomePage(): Promise<Result<HomePage, SystemError>> {
    try {
      ...
      return failure({ message: `EventsCompanion UI not available: ${response?.status()}`, fieldErrors: [] });
    } catch (e) {
      return failure({ message: `EventsCompanion UI not available: ${String(e)}`, fieldErrors: [] });
    }
  }
  ...
  private requirePage(): Page {
    if (!this.currentPage) {
      throw new Error('EventsCompanionUiClient: openHomePage() must be called before accessing pages');
    }
