...
// A10 — every EventsCompanionDriver operation takes a single *Request and returns Result<*Response, …> (strict).
test('A10 - every EventsCompanionDriver method takes a *Request and returns a *Response', () => {
  const violations: string[] = [];
  let found = false;

  for (const sf of sourceFiles) {
    const iface = sf.getInterface('EventsCompanionDriver');
    if (!iface) continue;
    found = true;
  ...
  expect(found, 'EventsCompanionDriver interface not found').toBe(true);
  expect(violations, `Every EventsCompanionDriver method must take a single *Request and return Result<*Response, …>: ${violations.join('; ')}`).toEqual([]);
});
