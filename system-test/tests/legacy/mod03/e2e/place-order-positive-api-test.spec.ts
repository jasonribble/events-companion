...
test('shouldPlaceOrderForValidInput', async ({ config }) => {
    ...
    const eventsCompanionApiUrl = config.eventsCompanion.backendApiUrl;
    ...
    const placeOrderResponse = await fetch(`${eventsCompanionApiUrl}/api/orders`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    const viewOrderResponse = await fetch(`${eventsCompanionApiUrl}/api/orders/${orderData.orderNumber}`);
