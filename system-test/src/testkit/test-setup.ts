...
import { EventsCompanionApiDriver } from './driver/adapter/api/events-companion-api-driver.js';
import { EventsCompanionUiDriver } from './driver/adapter/ui/events-companion-ui-driver.js';
...
export function createScenario(options: ScenarioOptions = {}): ScenarioDsl {
  ...
  const app = new AppContext({
    channelMode,
    channel,
    eventsCompanionDriverFactory: (ch) => createEventsCompanionDriverForChannel(config, ch as Channel, options),
...
function createEventsCompanionDriverForChannel(config: TestConfig, channel: Channel, options: ScenarioOptions) {
  if (channel === ChannelType.UI) {
    if (!options.browser) throw new Error('Browser is required for UI channel');
    return new EventsCompanionUiDriver(config.eventsCompanion.frontendUrl, options.browser);
  }
