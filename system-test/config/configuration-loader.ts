...
export function loadConfiguration(overrides?: ConfigOverrides): TestConfig {
  ...
  config.eventsCompanion.frontendUrl = getEnvVarOrDefault('MYSHOP_UI_BASE_URL' + suffix, config.eventsCompanion.frontendUrl);
  config.eventsCompanion.backendApiUrl = getEnvVarOrDefault('MYSHOP_API_BASE_URL' + suffix, config.eventsCompanion.backendApiUrl);
