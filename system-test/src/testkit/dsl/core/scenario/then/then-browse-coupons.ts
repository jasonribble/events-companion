...
export class ThenBrowseCouponsResultStage implements PromiseLike<void> {
  ...
  private async _doExecute(): Promise<void> {
    for (const cc of this.ctx.couponConfigs) {
      ...
      await this.app.eventsCompanion().publishCoupon({ code: resolvedCode, discountRate: String(cc.discountRate) });
    }

    const result = await this.app.eventsCompanion('static').browseCoupons({});
